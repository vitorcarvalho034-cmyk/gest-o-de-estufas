...
export default function Pautas() {
  ...
  async function load(sem, a) {
    ...
    try {
      ...
      const totalPrev = prevSem.reduce((s, p) => s + (p.pressas_previstas || 0), 0);
      setPrevHastes(totalPrev);

      // Anastasia 60h/80h por cesto
      const hAnast80 = prevSem.filter(p => isAnast80(p.variedade)).reduce((s, p) => s + (p.pressas_previstas||0), 0);
      const hAnast60 = prevSem.filter(p => isAnast60(p.variedade)).reduce((s, p) => s + (p.pressas_previstas||0), 0);
      ...
      setRealHastes(colSem.reduce((s, c) => s + (c.pressas || 0), 0));
